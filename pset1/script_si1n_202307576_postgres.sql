/*Apagar o database caso o mesmo já exista*/
DROP database if EXISTS uvv;

/*apagar o usuário caso o mesmo já exista*/
DROP USER IF EXISTS alan;


/*criação do usuário*/
CREATE USER alan WITH 
ENCRYPTED PASSWORD '123456' CREATEDB
CREATEROLE;
...
/*
INSTRUÇÕES PARA UTILIZAÇÃO DO BANCO
Após rodar o script com o comando: "psql -U postgres < si1n_202307576_postgres.sql" (a senha do usuário do postgres é: computacao@raiz), você já estará com tudo criado!
Para utilização basta entrar no banco de dados com o comando: "psql -U alan -d uvv";
Em seguida, para começar a utilizar o esquema "lojas" e fazer as seleções e edições, utilize o comando: "SET search_path TO lojas".
*/
...
/*Adição das restrições de integridade*/

ALTER TABLE lojas.produtos
ADD CONSTRAINT cc_preco_unitario_maior_que_zero CHECK (preco_unitario > 0);

ALTER TABLE lojas.pedidos_itens
ADD CONSTRAINT cc_preco_unitario_maior_que_zero CHECK (preco_unitario > 0);

ALTER TABLE lojas.pedidos_itens
ADD CONSTRAINT cc_quantidade_maior_que_zero CHECK (quantidade >= 0);
